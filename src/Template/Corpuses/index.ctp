<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('New Corpus'), ['action' => 'add']) ?></li>
        <li><?= $this->Html->link(__('Semantic Requests'), ['controller' => 'SemanticRequests', 'action' => 'index']) ?></li>
    </ul>
</nav>
<div class="corpuses index large-9 medium-8 columns content">
    <h3><?= __('Corpuses') ?></h3>
    <table cellpadding="0" cellspacing="0">
        <thead>
            <tr>
                <th scope="col"><?= $this->Paginator->sort('id') ?></th>
                <th scope="col"><?= $this->Paginator->sort('name') ?></th>
                <th scope="col"><?= $this->Paginator->sort('visiblis_number') ?></th>
                <th scope="col"><?= $this->Paginator->sort('created') ?></th>
                <th scope="col"><?= $this->Paginator->sort('updated') ?></th>
                <th scope="col" class="actions"><?= __('Actions') ?></th>
            </tr>
        </thead>
        <tbody>
            <?php foreach ($corpuses as $corpus): ?>
            <tr>
                <td><?= h($corpus->id) ?></td>
                <td><?= h($corpus->name) ?></td>
                <td><?= $this->Number->format($corpus->visiblis_number) ?></td>
                <td><?= h($corpus->created) ?></td>
                <td><?= h($corpus->updated) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $corpus->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $corpus->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $corpus->id], ['confirm' => __('Are you sure you want to delete # {0}?', $corpus->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
    <div class="paginator">
        <ul class="pagination">
            <?= $this->Paginator->prev('< ' . __('previous')) ?>
            <?= $this->Paginator->numbers() ?>
            <?= $this->Paginator->next(__('next') . ' >') ?>
        </ul>
        <p><?= $this->Paginator->counter() ?></p>
    </div>
</div>
